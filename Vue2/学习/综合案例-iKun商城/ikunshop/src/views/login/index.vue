      ...
      <div class="form">
        ...
        <div class="form-item">
          ...
          <button @click="getCode">
            {{ second === totalSecound ? '获取验证码' : second + '秒后重新发送' }}
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getCode () {
      ...
      if (!this.timer && this.second === this.totalSecound) {
        // 发送请求
        ...
        this.timer = setInterval(() => {
          ...
          if (this.second <= 0) {
            ...
            this.second = this.totalSecound // 归位
          }
        }, 1000)
      }
    },
    ...
    // 登录
    async login () {
      const regSmsCode = /^\d{6}$/
      if (!this.validFn()) {
        // 如果没通过校验, 就不往下走
        return
      }
      if (!regSmsCode.test(this.smsCode)) {
